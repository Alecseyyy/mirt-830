"""Mirtek CC1101 — ESPHome 2026.5.x"""
...
async def to_code(config):
    ...
    # Передаём cs_pin ещё раз как прямой GPIO для reset pulse
    cs_gpio = await cg.gpio_pin_expression(config[spi.CONF_CS_PIN])
    cg.add(var.set_cs_gpio(cs_gpio))
